...
class MagnetometerSensor(context: Context) : SensorSpec<Magnetometer> {
    override val sensorType: Int = SensorType.MAGNETOMETER.sensor
    override val requiredPermission: String? = SensorType.MAGNETOMETER.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "Magnetometer",
    ...
    override fun processSensorData(values: FloatArray): Magnetometer {
        return Magnetometer(values[0], values[1], values[2])
    }

    override fun createSensor(context: Context): BaseSensor<Magnetometer> {
