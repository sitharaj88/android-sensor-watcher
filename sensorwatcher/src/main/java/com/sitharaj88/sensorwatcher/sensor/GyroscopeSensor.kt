...
import com.sitharaj88.sensorwatcher.model.Gyroscope


class GyroscopeSensor(context: Context) : SensorSpec<Gyroscope> {
    override val sensorType: Int = SensorType.GYROSCOPE.sensor
    override val requiredPermission: String? = SensorType.GYROSCOPE.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "Gyroscope",
    ...
    override fun processSensorData(values: FloatArray): Gyroscope {
        return Gyroscope(values[0], values[1], values[2])
    }

    override fun createSensor(context: Context): BaseSensor<Gyroscope> {
