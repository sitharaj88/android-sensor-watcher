...
import com.sitharaj88.sensorwatcher.model.Gravity


class GravitySensor(context: Context) : SensorSpec<Gravity> {
    override val sensorType: Int = SensorType.GRAVITY.sensor
    override val requiredPermission: String? = SensorType.GRAVITY.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "Gravity",
    ...
    override fun processSensorData(values: FloatArray): Gravity {
        return Gravity(values[0], values[1], values[2])
    }

    override fun createSensor(context: Context): BaseSensor<Gravity> {
