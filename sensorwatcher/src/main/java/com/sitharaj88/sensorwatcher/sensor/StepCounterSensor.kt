...
import com.sitharaj88.sensorwatcher.model.StepCounter

@RequiresApi(Build.VERSION_CODES.Q)
class StepCounterSensor(context: Context) : SensorSpec<StepCounter> {
    override val sensorType: Int = SensorType.STEP_COUNTER.sensor
    override val requiredPermission: String? = SensorType.STEP_COUNTER.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "Step Counter",
    ...
    override fun processSensorData(values: FloatArray): StepCounter {
        return StepCounter(values[0])
    }

    override fun createSensor(context: Context): BaseSensor<StepCounter> {
