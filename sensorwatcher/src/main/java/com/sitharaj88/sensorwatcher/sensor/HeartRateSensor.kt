...
import com.sitharaj88.sensorwatcher.model.HeartRate

class HeartRateSensor(context: Context) : SensorSpec<HeartRate> {
    override val sensorType: Int = SensorType.HEART_RATE.sensor
    override val requiredPermission: String? = SensorType.HEART_RATE.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "HeartRate",
    ...
    override fun processSensorData(values: FloatArray): HeartRate {
        return HeartRate(values[0])
    }

    override fun createSensor(context: Context): BaseSensor<HeartRate> {
