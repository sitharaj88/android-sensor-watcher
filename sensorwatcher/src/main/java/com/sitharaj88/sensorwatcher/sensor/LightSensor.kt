...
import com.sitharaj88.sensorwatcher.model.LightSensor

class LightSensor(context: Context) : SensorSpec<LightSensor> {
    override val sensorType: Int = SensorType.LIGHT.sensor
    override val requiredPermission: String? = SensorType.LIGHT.requiredPermission

    override val sensorInfo: SensorInfo by lazy {
        val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val sensor = sensorManager.getDefaultSensor(sensorType)
        SensorInfo(
            name = sensor?.name ?: "Light",
    ...
    override fun processSensorData(values: FloatArray): LightSensor {
        return LightSensor(values[0])
    }

    override fun createSensor(context: Context): BaseSensor<LightSensor> {
